...
<div class="container">
    <div id="header">
        <h4>Guest Payment Information</h4>
    </div>
...
$(document).ready(function () {
    function loadTable(query = '',startDate='',endDate='') {
        $.ajax({
            url: 'search_payment.php',
            method: 'GET',
            dataType: 'json',
            data: { q: query,start:startDate,end:endDate },
            success: function (response) {
            $('#paymentTable tbody').html(response.html);
            // Store the totals for PDF use
            $('#downloadBtn').data('totals', {
                guests: response.totalGuests,
    ...
    document.getElementById("downloadBtn").addEventListener("click", function () {
        ...
        const totals = $(this).data('totals') || { guests: 0, amount: 0 };

        const startDate = $('#startDate').val() || 'N/A';
        const endDate = $('#endDate').val() || 'N/A';
        // Title
        doc.setFontSize(16);
        doc.text(`Guest Payment Information from ${startDate} to ${endDate}`, 14, 15);
        doc.setFontSize(12);
        doc.text(`Total Guests: ${totals.guests}`, 14, 25);
        doc.text(`Total Paid Amount: ${parseFloat(totals.amount).toFixed(2)} tk`, 14, 32)
        ...
        doc.autoTable({
            ...
            bodyStyles: { fillColor: [227, 242, 253] }
        });

        doc.save("guest_payment_information.pdf");
    });
});
</script>
