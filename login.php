...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            $role = $_SESSION['role'];
            if ($role === "teacher") {
                $redirect = "/Project/user/teacher/dashboard/teacher.php";
            } elseif ($role === "officestaff") {
                $redirect = "/php/office_staff.php";
            } elseif ($role === "register") {
                $redirect = "/Project/user/register/dashboard/register.php";
            } else {
        ...
        if ($attendant = $result->fetch_assoc()) {
            ...
            if (password_verify($password, $attendant['password'])) {
                ...
                echo json_encode([
                    "status" => "success",
                    "redirect" => "/Project/user/attendant/dashboard/attendant.php"
                ]);
            } else {
                echo json_encode([
                    "status" => "error",
